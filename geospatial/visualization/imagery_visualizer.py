...
class ImageryVisualizer(object):
    ...
    # using class variables to give default values works well so long as you do it with immutable values
    ...
    default_sentinel_2_viz_params = DefaultVisParams(
        min=0.0,
        max=0.3,
        gamma=0.7
    )

    def __init__(self, bands_vis_params: Union[str, dict] = None):
        if bands_vis_params is not None and isinstance(bands_vis_params, str):
            assert os.path.exists(bands_vis_params)
            with open(bands_vis_params) as f:
                bands_vis_params = json.load(f)

        self.bands_vis_params = bands_vis_params


    def show_single_band(self, raster: , size=(4, 4), cmap='gist_yarg'):
        raster = raster.squeeze()
        assert len(raster.shape) == 2
        fig = plt.figure(figsize=size)
        fig = plt.imshow(raster, cmap=cmap, norm=self.elevation_normalizer)
    ...
    @staticmethod
    def norm_band(bands, band_min=0, band_max=7000, gamma=1.0):
        """

        Args:
            bands: a (bands, w, h) tile
            band_min: minimum value the pixels are clipped tp
            band_max: maximum value the pixels are clipped to
            gamma: the gamma value to use in gamma correction. Output is darker for gamma > 1

        Returns:
            clipped, normalized and gamma corrected version of the single-band image,
    ...
    def show_landsat8_ndvi(self, tile_reader, window=None):
        """
        Computes the NDVI (Normalized Difference Vegetation Index) over a tile or a section
        on the tile specified by the window.

        NDVI values are between -1.0 and 1.0, mostly representing greenness, where any
        ...
        Args:
            ...
            window: a tuple of four (col_off x, row_off y, width delta_x, height delta_y)
                to specify the section of the tile to compute NDVI over

        Returns:
            2D numpy array of dtype float32 of the NDVI values at each pixel
            Returns all zeros if the sum of all pixels in the two bands is zero (likely because the section is empty)
        """
        if window:
        ...
        ndvi = (band_nir - band_red) / sum_red_nir  # each pixel in sum_red_nir could still be zero
        return ndvi


    def show_landsat8_tile(self, tile_reader, bands=bands_combo['visible'],
                           window=None,
                           band_min=0, band_max=7000, gamma=1,
                           size=(256, 256),
                           return_array=False):
        """
        ...
        Args:
            tile_reader: a rasterio.io.DatasetReader object returned by rasterio.open()
            bands: list of ints or a single int, indicating which band(s) to read. See notes
            ...
            band_max: maximum value the pixels are clipped to
            gamma: the gamma value to use in gamma correction. Output is darker for gamma > 1
            size: Used when this function is called to produce a visualization, i.e. only when
        ...
        Notes:
            - PIL renders the band in RGB order; keep that in mind when passing in `bands` as a list
              so that the desired band is mapped to Red, etc
        ...
        bands = VizUtils.norm_band(bands, band_min=band_min, band_max=band_max, gamma=gamma)
    ...
    def stat_landsat_tile(tile_tif, n_bins=40):
        ...
        # band index starts with a as in GDAL
